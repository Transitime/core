...
/**
 * For generating the JSON data for a Google chart for showing percent
 * of predictions that lie between an error range.
 *
 * @author SkiBu Smith
 *
 */
public class PredAccuracyRangeQuery extends PredictionAccuracyQuery {
	...
	builder.addNumberColumn("Within Bounds (" + maxEarlySec + " to " + maxLateSec + " secs)");
	builder.addTooltipColumn();
	...
	for (int predBucketIdx=0; 
		...
		// For this prediction bucket track whether prediction below min,
    ...
    public String getJson(String beginDateStr, String endDateStr,
	    ...
	    String predSource, String predType, int maxEarlySec,
	    int maxLateSec)
